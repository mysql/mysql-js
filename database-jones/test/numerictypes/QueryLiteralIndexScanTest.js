...
var udebug = unified_debug.getLogger("numerictypes/QueryLiteralIndexScanTest.js");
...
var q1 = {p1: 4, expected: [4, 5, 6], queryType: 2, ordered: false, predicate: function(qdt) {
  return qdt.tint.ge(qdt.param('p1')).and(qdt.tint.le(6));
}};

var q2 = {p1: 3, expected: [4, 5, 6], queryType: 2, ordered: false, predicate: function(qdt) {
  return qdt.tint.gt(qdt.param('p1')).and(qdt.tint.lt('7'));
}};

var q3 = {p1: 3, expected: [7], queryType: 2, ordered: false, predicate: function(qdt) {
  return qdt.tint.ne(qdt.param('p1')).and(qdt.tint.eq(7.0));
}};

var q4 = {p1: 8,  expected: [0, 1, 2, 8, 9], queryType: 2, ordered: false, predicate: function(qdt) {
  return qdt.tint.ge(qdt.param('p1')).or(qdt.tint.le('2.0'));
}};
...
var q5 = {p1: 3, expected: [7, 8, 9], queryType: 2, ordered: false, predicate: function(qdt) {
  ...
  fail_openSession(testCase, function(session) {
      ...
      queryTests.forEach(function(queryTest) {
        ...
        session.createQuery(from, function(err, q) {
          if (err) {
            testCase.appendErrorMessage('QueryLiteralIndexScanTest ' + queryTest.testName + ' returned error: ' + err);
            --testCount;
            return;
          }
            ...
            if(results) {
              ...
              if (queryTest.expected.length !== results.length) {
                ...
                for (j = 0; j < queryTest.expected.length; ++j) {
                  udebug.log_detail('QueryLiteralIndexScanTest ' + queryTest.testName + ' expected: '
